...
def add_to_wishlist(uid, isbn):
    res = manage_wishlist(True,uid,isbn)
    if "Error:" in res:
...
def view_books(app, member):
    def show_main_page():
        ...
        if len(Books)>0:
            for book in Books:
                ...
                title_label.bind("<Button-1>", lambda e, isbn=book["ISBN"]: show_details_page(isbn))
    ...
    def show_details_page(isbn):
        ...
        wishlist_button = ttk.Button(
            ...
            command=lambda:add_to_wishlist(member["UID"], isbn),
            style="crimson.TButton",
        )
